...
class BoundingBox(BaseModel):
    ...
    x1: float = Field(
        ..., description="Left edge x-coordinate in pixels", example=100.5, ge=0.0
    )
    y1: float = Field(
        ..., description="Top edge y-coordinate in pixels", example=150.2, ge=0.0
    )
    x2: float = Field(
        ..., description="Right edge x-coordinate in pixels", example=300.8, ge=0.0
    )
    y2: float = Field(
        ..., description="Bottom edge y-coordinate in pixels", example=400.1, ge=0.0
    )
    width: float = Field(
        ..., description="Width of the bounding box in pixels", example=200.3, ge=0.0
    )
    height: float = Field(
        ..., description="Height of the bounding box in pixels", example=249.9, ge=0.0
    )
...
class FeedbackAnnotation(BaseModel):
    ...
    class_id: int = Field(
        ..., description="YOLO class ID for the annotated object", example=15, ge=0
    )
    class_name: str = Field(
        ..., description="Human-readable name of the annotated class", example="cat"
    )
    ...
    correct_activity: Optional[CatActivity] = Field(
        None, description="User's correction of the detected activity"
    )
    ...
    corrected_class_name: Optional[str] = Field(
        None, description="Human-readable name of the corrected class", example="dog"
    )
