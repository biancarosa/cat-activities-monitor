...
class FeatureClusteringTrainer(BaseTrainer):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.clustering_methods = self.get_config(
    ...
    async def train(self, training_data: TrainingData) -> TrainingResult:
        ...
        try:
            ...
            target_components = min(
                self.get_config("pca_components", 256), max_components
            )
            ...
            self.logger.info(
                f"Reduced dimensions from {X.shape[1]} to {X_pca.shape[1]}"
            )
            self.logger.info(
            ...
            metrics = {
                ...
                "clustering_results": self._make_clustering_results_serializable(
                    clustering_results
                ),
            }
            ...
            self.logger.info(
                f"Best method: {best_method} (silhouette: {best_score:.3f})"
            )
    ...
    async def _train_kmeans(
        self, X: np.ndarray, y_true: List[str], k_range: range
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if mask.sum() > 0:
                    ari_score = adjusted_rand_score(
                        y_true_encoded[mask], y_pred_best[mask]
                    )
            except Exception:
                pass
    ...
    async def _train_agglomerative(
        self, X: np.ndarray, y_true: List[str], k_range: range
    ) -> Dict[str, Any]:
    ...
    async def validate_training_data(self, training_data: TrainingData) -> bool:
        ...
        min_unique_samples = self.get_config(
            "min_unique_samples", 3
        )  # Lower default for small datasets
        if len(unique_features) < min_unique_samples:
            self.logger.error(
                f"Need at least {min_unique_samples} unique feature vectors for clustering, got {len(unique_features)}"
            )
            return False
        ...
        self.logger.info(
            f"Clustering validation passed: {len(unique_features)} unique samples"
        )
        return True

    async def predict_clusters(
        self, features: List[List[float]], model_path: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            cluster_counts = {
                label: list(cluster_labels).count(label) for label in unique_clusters
            }
    ...
    def _make_clustering_results_serializable(
