...
class MLTrainingPipeline:
    ...
    async def initialize(self) -> None:
        ...
        """
        logger.info(f"Initializing ML training pipeline with {len(self.trainers)} trainers")
        
        for i, trainer in enumerate(self.trainers):
            if not trainer.is_initialized():
                trainer_name = trainer.get_trainer_name()
                logger.info(f"Initializing trainer {i+1}/{len(self.trainers)}: {trainer_name}")
                try:
    ...
    async def _train_sequential(self, training_data: TrainingData) -> Dict[str, TrainingResult]:
        ...
        for i, trainer in enumerate(self.trainers):
            ...
            try:
                ...
                if trainer_name == "YOLOTrainer":
                    sample_count = len(training_data.metadata) if training_data.metadata else 0
                else:
                    sample_count = len(training_data.features) if training_data.features else 0
                ...
                if result.success:
                    ...
                    logger.error(f"Training failed for {trainer_name}: {result.error_message}")
    ...
    async def _train_parallel(self, training_data: TrainingData) -> Dict[str, TrainingResult]:
    ...
    async def cleanup(self) -> None:
        ...
        for trainer in self.trainers:
            try:
                ...
                logger.error(f"Error setting up cleanup for {trainer.get_trainer_name()}: {e}")
