...
class YOLOTrainer(BaseTrainer):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.batch_size = self.get_config(
            "batch_size", 4
        )  # Smaller batch for small datasets
    ...
    async def initialize(self) -> None:
        ...
        if not base_model_path.exists():
            self.logger.warning(
                f"Base model {self.base_model} not found at {base_model_path}"
            )
        ...
        try:
            ...
            if not training_prepared["success"]:
                return TrainingResult(
                    success=False, error_message=training_prepared["error"]
                )
            ...
            if training_result["success"]:
                self.logger.info(
                    f"YOLO training completed successfully in {training_time:.2f}s"
                )
                return TrainingResult(
                    success=True,
    ...
    async def _prepare_yolo_training_data(
        ...
        try:
            ...
            for metadata in training_data.metadata:
                try:
                    ...
                    if source_image_path and Path(source_image_path).exists():
                        ...
                        with open(label_file, "w") as f:
                            ...
                            for annotation in user_annotations:
                                ...
                                corrected_class_id = annotation.get(
                                    "corrected_class_id"
                                )
                                original_class_id = annotation.get(
                                    "class_id", 15
                                )  # Default to cat
                                ...
                                if corrected_class_id == -1:  # Rejection marker
                                    self.logger.debug(
                                        "Skipping rejected annotation in YOLO training"
                                    )
                                    continue
                                ...
                                if bbox and "x1" in bbox and "x2" in bbox:
                                    ...
                                    width = (
                                        bbox.get("width", bbox["x2"] - bbox["x1"])
                                        / img_width
                                    )
                                    height = (
                                        bbox.get("height", bbox["y2"] - bbox["y1"])
                                        / img_height
                                    )
                                    ...
                                    if (
                                        ...
                                        self.logger.info(
                                            f"🔄 YOLO class correction: {original_class_name} (id={original_class_id}) -> {corrected_class_name} (id={class_id})"
                                        )
                    ...
                    self.logger.warning(
                        f"Error processing training sample {feedback_id}: {e}"
                    )
                    continue
            ...
            if exported_count == 0:
                return {"success": False, "error": "No training data could be prepared"}

            self.logger.info(
                f"Prepared YOLO training data: {exported_count} images, {annotation_count} annotations"
            )
    ...
    async def _run_yolo_training(
        self, training_prepared: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            train_args = [
                ...
                "data=training_data/dataset.yaml",  # Use relative path
            ...
            try:
                ...
                return {"success": False, "error": "Training timeout (exceeded 1 hour)"}
            ...
            if result.returncode == 0:
                ...
                if best_model.exists():
                    ...
                    return {
                        ...
                        "metrics": {
                            "training_images": training_prepared["images_count"],
                            "training_annotations": training_prepared[
                                "annotations_count"
                            ],
                            "epochs_completed": self.epochs,
                            "model_size_mb": round(
                                final_model_path.stat().st_size / (1024 * 1024), 2
                            ),
                        },
                    }
                else:
    ...
    async def validate_training_data(self, training_data: TrainingData) -> bool:
        ...
        for metadata in training_data.metadata:
            if (
                ...
                and metadata.get("image_filename")
                and metadata.get("feedback_data")
            ):
                valid_samples += 1
        ...
        if valid_samples < min_samples:
            self.logger.error(
                f"Only {valid_samples} valid samples, need at least {min_samples}"
            )
            return False

        self.logger.info(
            f"YOLO training validation passed: {valid_samples} valid samples"
        )
        return True
