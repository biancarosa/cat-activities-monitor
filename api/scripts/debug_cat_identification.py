...
class CatIdentificationDebugger:
    ...
    async def initialize(self):
        ...
        self.cat_identification_service = CatIdentificationService(self.database_service)
    ...
    async def debug_image(self, image_filename: str, expected_cat_name: Optional[str] = None):
        """Debug cat identification for a specific image"""
        
        print("=" * 80)
        print(f"🔍 DEBUGGING CAT IDENTIFICATION")
        ...
        try:
            async with self.database_service.get_session() as session:
                result = await session.execute(
                    text("SELECT image_filename, cats_count, confidence, detections, timestamp "
                ...
                if row:
                    ...
                    print(f"   🎯 Max confidence: {row[2]:.3f}" if row[2] else "   🎯 Max confidence: None")
                    ...
                    for i, detection in enumerate(detections):
                        ...
                        print(f"         Has Features: {'Yes' if detection.get('features') else 'No'}")
                        if detection.get('features'):
                            print(f"         Feature Length: {len(detection['features'])}")
                else:
    ...
    async def _check_cat_profiles(self, expected_cat_name: Optional[str] = None):
        ...
        try:
            async with self.database_service.get_session() as session:
                result = await session.execute(
                    text("SELECT cat_uuid, name, feature_template, total_detections, average_confidence "
                ...
                for profile in profiles:
                    ...
                    if expected_cat_name and name.lower() == expected_cat_name.lower():
                        print(f"      🎯 This is the expected cat!")
                    print()
                
                print(f"📊 Summary: {profiles_with_features}/{len(profiles)} profiles have features")
                
                if profiles_with_features == 0:
                    print("⚠️ WARNING: No profiles have features - identification will not work!")
                
        except Exception as e:
            print(f"❌ Error checking cat profiles: {e}")

    async def _extract_features_from_image(self, image_path: Path) -> Optional[List[Detection]]:
    ...
    async def _test_identification(self, detections: List[Detection], expected_cat_name: Optional[str] = None):
        ...
        try:
            ...
            print(f"✅ Identification completed for {len(identification_results)} detections")
            ...
            for i, result in enumerate(identification_results):
                ...
                if suggested_profile:
                    ...
                    if expected_cat_name:
                        is_correct = suggested_profile['name'].lower() == expected_cat_name.lower()
                        status = "✅ CORRECT" if is_correct else "❌ INCORRECT"
                        print(f"      Expected match: {status}")
                else:
                    print(f"      Suggested cat: None (new cat)")
                ...
                if all_matches:
                    ...
                    for j, match in enumerate(all_matches[:3]):
                        ...
                        print(f"         {j+1}. {match['profile']['name']}: "
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: python debug_cat_identification.py <image_filename> [expected_cat_name]")
        print("\nExamples:")
        print("  python debug_cat_identification.py living-room_20250621_155350_activity_detections.jpg Chico")
        print("  python debug_cat_identification.py living-room_20250621_155350_activity_detections.jpg")
        sys.exit(1)
