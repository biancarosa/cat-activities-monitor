...
async def get_detection_images(request: Request, page: int = 1, limit: int = 20):
    ...
    try:
        ...
        paginated_results = await database_service.get_paginated_detection_results(
            page, limit
        )
        ...
        for row in rows:
            ...
            image_file = (
                detection_imgs_path / image_filename if image_filename else None
            )
            ...
            detections = (
                ...
                else (json.loads(row["detections"]) if row["detections"] else [])
            )
                ...
                for i, ann in enumerate(user_annotations):
                    ...
                    if ann.get("activity_feedback"):
                        summary_parts.append(
                            f"Notes: {ann['activity_feedback'][:30]}..."
                        )
                    if summary_parts:
                        annotation_summary.append(
                            f"User annotation {i+1}: {', '.join(summary_parts)}"
                        )
            image_info = {
                ...
                "cat_count": cat_count,
                "max_confidence": (
                    round(max_confidence, 3) if max_confidence is not None else None
                ),
                "has_feedback": image_feedback is not None,
                "has_detailed_annotations": image_feedback is not None,
                "inference_method": (
                    "user_feedback" if image_feedback else "database_data"
                ),
                "detections": detections[
                    :3
                ],  # Limit to first 3 detections for performance
                "annotation_summary": annotation_summary[
                    :5
                ],  # Limit to 5 summary items
                "detection_info": {
                    "max_confidence": (
                        round(max_confidence, 3) if max_confidence is not None else None
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get detection images: {str(e)}"
        )


@router.get(
    "/images/{image_filename}/annotations",
...
async def get_image_annotations(request: Request, image_filename: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get annotations: {str(e)}"
        )
...
@router.post(
    "/images/{image_filename}/reprocess",
...
async def reprocess_detection_image(request: Request, image_filename: str):
    ...
    try:
        ...
        detection_result = await detection_service.detect_objects_with_activity(
            image_data, yolo_config, source_name
        )

        # Delete existing database record for this image
        deleted_count = await database_service.delete_detection_result_by_filename(
            image_filename
        )
        ...
        if detection_result.detected:
            ...
            await database_service.save_detection_result(
                source_name, detection_result, image_array, image_filename
            )
            logger.info(
                f"✅ Reprocessed and saved detection for {image_filename}: {detection_result.cats_count} cats detected"
            )
        else:
        ...
        return {
            ...
            "detection_result": {
                ...
                "confidence": (
                    round(detection_result.confidence, 3)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reprocess detection: {str(e)}"
        )


@router.post(
    "/images/reprocess-all",
...
async def reprocess_all_detection_images(request: Request):
    ...
    try:
        ...
        if not detection_imgs_path.exists():
            return {
                "message": "No detection images directory found",
                "processed": 0,
                "errors": 0,
            }
        ...
        logger.info(
            f"🔄 Starting bulk reprocessing of {len(image_files)} detection images"
        )
        ...
        for image_file in image_files:
            try:
                ...
                # Delete existing database record for this image
                deleted_count = (
                    await database_service.delete_detection_result_by_filename(
                        image_file.name
                    )
                )
                ...
                if detection_result.detected:
                    ...
                    await database_service.save_detection_result(
                        source_name, detection_result, image_array, image_file.name
                    )
                ...
                if processed_count % 10 == 0:
                    logger.info(
                        f"📊 Bulk reprocessing progress: {processed_count}/{len(image_files)} images processed"
                    )

            except Exception as e:
                logger.warning(f"Error reprocessing {image_file.name}: {e}")
                results.append(
                    {"filename": image_file.name, "success": False, "error": str(e)}
                )
                error_count += 1

        logger.info(
            f"✅ Bulk reprocessing completed: {processed_count} processed, {error_count} errors"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to bulk reprocess: {str(e)}"
        )
