...
class UnifiedTrainingRequest(BaseModel):
    ...
    train_yolo: bool = Field(
        True, description="Whether to train YOLO object detection model"
    )
    train_cat_identification: bool = Field(
        True, description="Whether to train cat identification models"
    )
    include_clustering: bool = Field(
        True, description="Whether to include clustering analysis"
    )
    parallel_training: bool = Field(
        False, description="Whether to run trainers in parallel"
    )
...
async def get_training_status(request: Request) -> TrainingStatusResponse:
    ...
    try:
        ...
        yolo_ready = (
            feedback_stats["total_feedback"] >= 5
            and feedback_stats["total_annotations"] >= 10
        )

        cat_id_ready = (
            feedback_stats["unique_cats"] >= 2
            and feedback_stats["cat_profiles"] >= 2
            and feedback_stats["total_annotations"] >= 20
        )
    ...
    try:
            ...
            yolo_trainer = YOLOTrainer(
                {
                    ...
                    "base_model": training_request.yolo_config.get(
                        "base_model", "yolo11l.pt"
                    ),
                    "model_dir": "ml_models",
        ...
        if training_request.train_cat_identification:
            ...
            if training_data.features:
                ...
                min_samples_per_cat = (
                    max(2, min(3, total_samples // unique_cats))
                    if unique_cats > 0
                    else 2
                )
        ...
        training_results = await pipeline.train_all(
            training_data, parallel=training_request.parallel_training
        )
        ...
        response = {
            ...
            "training_data_stats": {
                ...
                "unique_labels": (
                    len(set(training_data.labels)) if training_data.labels else 0
                ),
            },
        }
...
async def suggest_cat_identifications(request: Request) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(
            f"Generated {len(identification_results)} cat identification suggestions"
        )
...
async def update_cat_profile_features(
    cat_profile_uuid: str, request: Request
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Cat profile not found or update failed"
            )
            ...
            for model_file in models_dir.glob("*.pt"):
                ...
                if metadata_file.exists():
                    try:
                        ...
                        logger.warning(
                            f"Error reading metadata for {model_file.name}: {e}"
                        )
...
async def _extract_training_data_from_feedback(
    ...
    try:
        async with database_service.get_session() as session:
            ...
            return TrainingData(features=features, labels=labels, metadata=metadata)
...
async def _extract_features_from_detection(
    ...
    try:
        ...
        for detection in original_detections:
            ...
            if (
                features
                and len(features) == 2048
                and abs(detection_bbox.get("x1", 0) - annotation_bbox.get("x1", 0)) < 5
                and abs(detection_bbox.get("y1", 0) - annotation_bbox.get("y1", 0)) < 5
            ):
...
async def _extract_features_from_image_crop(
    bbox: Dict[str, Any], image_path: str
) -> List[float]:
    """
                ...
                with open(label_file, "w") as f:
                    ...
                    for annotation in user_annotations:
                        ...
                        if corrected_class_id == -1:  # Rejection marker
                            logger.debug(
                                "Skipping rejected annotation in training export"
                            )
                            continue
                        ...
                        if bbox and "x1" in bbox:
                            ...
                            width = (
                                bbox.get("width", bbox["x2"] - bbox["x1"]) / img_width
                            )
                            height = (
                                bbox.get("height", bbox["y2"] - bbox["y1"]) / img_height
                            )
                            ...
                            if (
                                corrected_class_id is not None
                                and corrected_class_id != original_class_id
                            ):
                                corrected_class_name = annotation.get(
                                    "corrected_class_name", "unknown"
                                )
                                original_class_name = annotation.get(
                                    "class_name", "unknown"
                                )
                                logger.info(
                                    f"🔄 Class correction applied: {original_class_name} (id={original_class_id}) -> {corrected_class_name} (id={class_id})"
                                )
                        else:
                            logger.warning(
                                f"Invalid bounding box in annotation: {bbox}"
                            )
