...
async def submit_feedback(request: Request, feedback: ImageFeedback):
    ...
    try:
        ...
        if image_path.startswith("/detections/"):
            ...
            if config:
                actual_path = (
                    Path(config.global_.ml_model_config.detection_image_path) / filename
                )
            else:
        ...
        if not actual_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Image file not found: {actual_path}"
            )

        # Generate feedback ID
        feedback_id = hashlib.md5(
            f"{feedback.image_filename}_{feedback.timestamp}".encode()
        ).hexdigest()

        # Prepare feedback data for database
        feedback_data = feedback.model_dump()
        feedback_data["image_path"] = str(
            actual_path
        )  # Store the actual file system path
        ...
        if feature_extractor:
            try:
                ...
                logger.info(
                    f"Loaded detection image for feature extraction: {actual_path}"
                )
            except Exception as e:
        ...
        for i, annotation in enumerate(feedback.user_annotations):
            ...
            if annotation.cat_profile_uuid:
                existing_profile = await database_service.get_cat_profile_by_uuid(
                ...
                if existing_profile is not None:
                    ...
                    profile["average_confidence"] = (
                        (old_avg * (total_detections - 1)) + new_confidence
                    ) / total_detections
                    ...
                    if annotation.correct_activity:
                        ...
                        activity_value = (
                            annotation.correct_activity.value
                            if hasattr(annotation.correct_activity, "value")
                            else annotation.correct_activity
                        )
                    ...
                    if (
                        feature_extractor
                        and detection_image is not None
                        and i < len(feedback.original_detections)
                    ):
                        try:
                            detection = feedback.original_detections[i]
                            if (
                                hasattr(detection, "bounding_box")
                                and detection.bounding_box
                            ):
                                ...
                                x2, y2 = int(
                                    bbox.get("x2", detection_image.shape[1])
                                ), int(bbox.get("y2", detection_image.shape[0]))
                                ...
                                if x2 > x1 and y2 > y1 and x1 >= 0 and y1 >= 0:
                                    ...
                                    # Extract features
                                    features = feature_extractor._extract_features(
                                        cat_crop
                                    )

                                    if features is not None and len(features) > 0:
                                        # Update cat profile with features using ensemble averaging
                                        cat_identification_service = (
                                            request.app.state.cat_identification_service
                                        )
                                        if cat_identification_service:
                                            async with database_service.get_session() as session:
                                                logger.info(
                                                    f"Updating cat profile {annotation.cat_profile_uuid} with features of length {len(features.tolist())}"
                                                )
                                                ...
                                                logger.info(
                                                    f"Update success: {success}"
                                                )
                                        else:
                                            logger.warning(
                                                "Cat identification service not available for feature updates"
                                            )
                                    else:
                                        logger.warning(
                                            f"Failed to extract features for {profile.get('name')}"
                                        )
                                else:
                                    logger.warning(
                                        f"Invalid bounding box for {profile.get('name')}: {bbox}"
                                    )
                            else:
                                logger.warning(
                                    f"No bounding box available for detection {i}"
                                )
                        except Exception as e:
                            logger.error(
                                f"Error extracting features for {profile.get('name')}: {e}"
                            )

                    await database_service.save_cat_profile(profile)
                    logger.info(
                        f"🐱 Updated cat profile: {profile.get('name')} (Total detections: {profile['total_detections']})"
                    )
        ...
        logger.info(
            f"📝 Feedback submitted for {feedback.image_filename}: {feedback.feedback_type}"
        )
        ...
        profiles_with_features = sum(
            1 for ann in feedback.user_annotations if ann.cat_profile_uuid
        )
        logger.info(f"   Cat profiles updated with features: {profiles_with_features}")
        logger.info(
            f"   Activity feedback: {[ann.activity_feedback for ann in feedback.user_annotations if ann.activity_feedback]}"
        )
        ...
        return {
            ...
            "activity_feedback_count": len(
                [ann for ann in feedback.user_annotations if ann.activity_feedback]
            ),
...
async def delete_feedback(request: Request, feedback_id: str):
    ...
    try:
        ...
        logger.info(
            f"🗑️ Deleted feedback for {feedback_data['image_filename']} (ID: {feedback_id})"
        )
