...
async def fetch_camera_image(request: Request, camera_name: str):
    ...
    try:
        ...
        result = await image_service.fetch_single_image(
            image_config, config.global_.ml_model_config, config.global_.timeout_seconds
        )
        ...
        if result["success"]:
            ...
            logger.error(
                f"❌ Failed to fetch '{camera_name}': {result.get('error', 'Unknown error')}"
            )
            raise HTTPException(
                status_code=500,
                detail=f"Failed to fetch '{camera_name}': {result.get('error', 'Unknown error')}",
            )
            ...
            "results": [
                {
                    ...
                    **(
                        ...
                        if r["success"] and "detection" in r
                        else {}
                    ),
                    **(
