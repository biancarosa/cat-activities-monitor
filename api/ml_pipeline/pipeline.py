    ...
    async def initialize(self) -> None:
        ...
        for i, process in enumerate(self.processes):
            if not process.is_initialized():
                logger.info(
                    f"Initializing process {i+1}/{len(self.processes)}: {process.get_process_name()}"
                )
    ...
    async def process_image(self, image_array: np.ndarray) -> ImageDetections:
        ...
        for i, process in enumerate(self.processes):
            ...
            try:
                ...
                logger.debug(
                    f"Process {process_name} completed. Cats detected: {detections.cats_count}"
                )
    ...
    async def cleanup(self) -> None:
        ...
        for process in self.processes:
            try:
                ...
                logger.error(
                    f"Error cleaning up process {process.get_process_name()}: {e}"
                )
