...
class ContextualActivityDetectionProcess(MLDetectionProcess):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.interaction_thresholds = self.activity_config.get(
            "interaction_thresholds", {}
        )

        # Default thresholds
        self.proximity_distance = self.interaction_thresholds.get(
            "proximity_distance", 100.0
        )
        self.overlap_threshold = self.interaction_thresholds.get(
            "overlap_threshold", 0.1
        )
        self.eating_confidence = self.interaction_thresholds.get(
            "eating_confidence", 0.8
        )
        self.sleeping_confidence = self.interaction_thresholds.get(
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Proximity distance threshold: {self.proximity_distance} pixels"
            )
            logger.info(f"Overlap threshold: {self.overlap_threshold}")
            logger.info(
                f"Target contextual objects: {list(self.contextual_objects.values())}"
            )

            self._set_initialized()
            logger.info(
                "Contextual Activity Detection Process initialized successfully"
            )
    ...
    async def process(
        ...
        try:
            ...
            logger.debug(
                f"Processing {len(cats)} cats with {len(objects)} contextual objects"
            )
            ...
            for cat in cats:
                if cat.class_name == "cat":  # Only process cats, not dogs
                    ...
                    activity_info = self._classify_contextual_activity(
                        cat, nearby_objects
                    )
            ...
            logger.debug(
                f"Contextual activity detection completed for {len(cats)} cats"
            )
            return detections
    ...
    def _find_nearby_objects(
        self, cat: Detection, objects: List[Detection]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for obj in objects:
            ...
            if distance <= self.proximity_distance:
                relationship = self._analyze_spatial_relationship(
                    cat.bounding_box, obj.bounding_box
                )
    ...
    def _analyze_spatial_relationship(
        self, cat_bbox: Dict, obj_bbox: Dict
    ) -> Dict[str, Any]:
        """
        ...
        cat_on_object = cat_bbox["y2"] >= obj_bbox["y1"] and cat_bbox["y1"] <= obj_bbox[
            "y1"
        ] + (obj_bbox["height"] * 0.3)
    ...
    def _determine_interaction_type(
        self, cat: Detection, obj: Detection, relationship: Dict
    ) -> str:
        """
    ...
    def _classify_contextual_activity(
        self, cat: Detection, nearby_objects: List[Dict]
    ) -> Dict[str, Any]:
        """
        ...
        pose_activity["contextual_description"] = (
            f"near {best_interaction['object_class']}"
        )
