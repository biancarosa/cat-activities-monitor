...
class YOLODetectionProcess(MLDetectionProcess):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.yolo_config.save_detection_images:
                Path(self.yolo_config.detection_image_path).mkdir(
                    parents=True, exist_ok=True
                )
                logger.info(
                    f"Detection images will be saved to: {self.yolo_config.detection_image_path}"
                )
    ...
    async def process(
        ...
        try:
            ...
            for result in results:
                if result.boxes is not None:
                    for box in result.boxes:
                        ...
                        contextual_object_classes = getattr(
                            self.yolo_config, "contextual_objects", []
                        )
                        ...
                        if is_cat or is_contextual:
                            ...
                            detection = Detection(
                                ...
                                cat_uuid=(
                                    str(uuid.uuid4()) if is_cat else None
                                ),  # Only cats get UUIDs
                            )
            ...
            logger.debug(
                f"YOLO detected {len(cat_detections)} cats and {len(contextual_objects)} contextual objects with max confidence {max_confidence:.3f}"
            )
