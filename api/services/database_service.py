...
class DatabaseService:
    ...
    def __init__(self, database_url: str = None):
        ...
        self.pool = None
        self.migration_service = MigrationService(self.database_url)
    ...
    async def init_database(self):
        ...
        # Use migration service to ensure database is ready
        success = await self.migration_service.ensure_database_ready()
        if success:
            logger.info(f"📊 Database initialized via migrations: {self.database_url}")
        else:
            logger.error(f"❌ Failed to initialize database: {self.database_url}")
