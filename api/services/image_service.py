...
class ImageService:
    ...
    async def fetch_image_from_url(
        self, image_config: ImageConfig, yolo_config, timeout: int = 30
    ) -> Tuple[bool, Optional[ImageDetections]]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                logger.info(
                    f"Fetching image '{image_config.name}' from URL: {image_config.url}"
                )
                ...
                if content_type.startswith("application/json"):
                    try:
                        ...
                        if "message" in json_data and json_data["message"].startswith(
                            "http"
                        ):
                            # This is likely a URL to an actual image, fetch that too
                            image_url = json_data["message"]
                            logger.info(
                                f"Found image URL in JSON response: {image_url}"
                            )
                            ...
                            logger.warning(
                                "JSON response doesn't contain expected image URL structure"
                            )
                            return False, None
                    ...
                    logger.warning(
                        f"URL '{image_config.name}' did not return an image. Content-Type: {content_type}"
                    )
                    return False, None

                image_size = len(image_data)
                logger.info(
                    f"Successfully fetched image '{image_config.name}'! Size: {image_size} bytes, Content-Type: {content_type}"
                )
                ...
                if self.detection_service.ml_pipeline and yolo_config:
                    try:
                        detection_result = (
                            await self.detection_service.detect_objects_with_activity(
                                image_data, yolo_config, image_config.name
                            )
                        )
                        ...
                        if detection_result.cat_detected:
                            ...
                            target_classes = [
                                COCO_CLASSES.get(cls, f"class_{cls}")
                                for cls in yolo_config.target_classes
                            ]
                            logger.info(
                                f"🎯 TARGET OBJECTS DETECTED in '{image_config.name}'! "
                            ...
                            logger.info(
                                f"No target objects detected in '{image_config.name}' "
                        ...
                        try:
                            ...
                            logger.debug(
                                f"💾 Saved detection result to database: {image_config.name} -> {image_filename}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Failed to save detection result to database: {e}"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error during object detection for '{image_config.name}': {e}"
                        )
            ...
            logger.error(
                f"Timeout while fetching image '{image_config.name}' from {image_config.url}"
            )
            return False, None
        except httpx.HTTPStatusError as e:
            logger.error(
                f"HTTP error {e.response.status_code} while fetching image '{image_config.name}' from {image_config.url}"
            )
            return False, None
        except Exception as e:
            logger.error(
                f"Unexpected error while fetching image '{image_config.name}' from {image_config.url}: {str(e)}"
            )
            return False, None

    async def fetch_all_images(
        self, image_configs, yolo_config, max_concurrent: int = 5, timeout: int = 30
    ):
        ...
        async def fetch_with_semaphore(img_config):
            async with semaphore:
                success, detection_result = await self.fetch_image_from_url(
                    img_config, yolo_config, timeout
                )
                result = {
