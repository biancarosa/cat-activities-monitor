    ...
    def load_config(self, config_path: str = "config.yaml") -> Config:
        ...
        try:
            ...
            if config_file.exists():
                ...
                logger.info(
                    f"Loaded configuration from file with {len(self._config.images)} image sources"
                )
            else:
                logger.info(
                    f"Configuration file {config_path} not found, using code defaults"
                )
                # Use default configuration from the model defaults
                self._config = Config()
                logger.info(
                    "Using default configuration with no image sources (add config.yaml to configure cameras)"
                )

            logger.info(
                f"ML Model Config: Model={self._config.global_.ml_model_config.model}, "
