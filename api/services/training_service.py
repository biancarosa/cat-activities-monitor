...
from models import ModelSaveRequest, ModelRetrainRequest
...
class TrainingService:
    ...
    def save_current_model(
        self, model_save_request: ModelSaveRequest, ml_model
    ) -> Dict:
        ...
        try:
            ...
            metadata = {
                ...
                "original_model": (
                    str(ml_model.model_path)
                    if hasattr(ml_model, "model_path")
                    else "unknown"
                ),
            ...
            metadata_path = (
                metadata_dir
                / f"{model_save_request.model_name}_{timestamp}_metadata.json"
            )
    ...
    def retrain_model_enhanced(
        self, retrain_request: ModelRetrainRequest, ml_model, yolo_config
    ) -> Dict:
        """
        ...
        try:
            ...
            if not training_dir.exists() or not dataset_yaml.exists():
                raise ValueError(
                    "Enhanced training data not found. Please export enhanced training data first."
                )
            ...
            if image_count == 0:
                raise ValueError(
                    "No training images found. Please provide feedback first."
                )

            if label_count == 0:
                raise ValueError(
                    "No training labels found. Please provide feedback with corrections."
                )
            ...
            if best_model_path.exists():
                ...
                logger.info(
                    f"🐱 Cat identification data: {len(cat_identification_data)} named cats"
                )
                ...
                raise FileNotFoundError(
                    f"Best enhanced model not found at {best_model_path}"
                )
    ...
    def list_saved_models(self) -> List[Dict]:
        ...
        for metadata_file in models_dir.glob("*_metadata.json"):
            try:
                ...
                model_path = (
                    ...
                    or models_dir / f"{metadata_file.stem.replace('_metadata', '')}.pt"
                )
        ...
        models.sort(
            key=lambda x: x.get("saved_timestamp", x.get("training_timestamp", "")),
            reverse=True,
        )
